...
public class Histogram {
	...
	public static void main(String[] args) {
		...
		char shade = '\u2592';
		String stringShade = String.valueOf(shade);
		printHistogram(results, valueMin, stringShade);
...
//		printHistogram(results, valueMin);
	...
	private static void printHistogram(int[] result, int minValue, String shade) {
		for (int i = 0; i < result.length; i++) {
			System.out.printf("%d - %d", minValue, minValue + INTERVAL_NUMBER);
			System.out.printf("%s%n", shade.repeat(result[i]));
			minValue += INTERVAL_NUMBER;
		}
	}
